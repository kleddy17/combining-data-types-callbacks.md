...
// here we have a function called f taking a parameter l
const f = l => {
  ...
  while (c <= l) {
    ...
    n = c + p; 
    //setting n to the valoue of c and p
    [c, p] = [n, c]
    es += (c % 2 === 0) ? c : 0
    //tenary conditonal statement the question mark is an if statement
    // the two dots are the else
    //real shorthand to write a conditonal
  }
  return es
}
...
// At first we may strive for the shortest code possible. But we really should be striving
// for readable code that is easy to maintain. 
//Compare the same solution with more semantic variable names
